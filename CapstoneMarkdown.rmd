...
##### Let's take a look at a few models and then select the best. The goal is to end up with a predictive model that is as simple as possible (i.e. fewest number of variables required)
```{r comment=""}
summary(lm(Revenue ~ ., data = VTT_Train))
```  

##### The adjusted $r^{2}$ looks fantastic, but lets remove Percent_Exit (the least significant variable):

```{r comment=""}
summary(lm(Revenue ~ . - Percent_Exit, data = VTT_Train))
```  

##### The $r^{2}$ is essentially the same and there are still some insignificant variables. Let's remove Year (the next least significant variable):

```{r comment=""}
summary(lm(Revenue ~ . - Percent_Exit - Year, data = VTT_Train))
```  

##### The adjusted $r^{2}$ remains virtually the same and all the variables are significant. Let's keep removing the least significant variables one by one and re-evaluating the $r^{2}$ as we go.  The below model has the Week_Index removed:

```{r comment=""}
summary(lm(Revenue ~ . - Percent_Exit - Year - Week_Index, data = VTT_Train))
```  

##### The adjusted $r^{2}$ is still very high. Let's remove Bounce_Rate and keep going:

```{r comment=""}
summary(lm(Revenue ~ . - Percent_Exit - Year - Week_Index - Bounce_Rate, data = VTT_Train))
```  

##### Still a great model, let's remove SkuCount:  

```{r comment=""}
summary(lm(Revenue ~ . - Percent_Exit - Year - Week_Index - Bounce_Rate - SkuCount, data = VTT_Train))
```  

##### Let's run with this one. The adjusted $r^{2}$ is very high and this model only requires three variables.   
